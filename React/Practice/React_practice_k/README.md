...
This looks like the HTML we know and love. It's much more readable and intuitive. JSX is not able to be read directly in the browser. It needs to be converted, or **compiled**, into plain JavaScript. To do this we'll use Vite.
...
[Vite](https://vite.dev/) is a bundler and multipurpose build tool for modern JavaScript/TypeScript applications. When we run `vite`, it recursively traverses your codebase, gathering any and all files that need to be sent to the browser, and then starts up a development server in order to provide those files upon request. In addition to this handy server, one of the main jobs we are using Vite for is compiling all our TypeScript files into JavaScript; that way, we don't need to worry about the compilation ourselves and can focus on other concerns. Vite can do a lot more than this though: for example, we're also using it to take React Components written in [JSX Syntax](https://react.dev/learn/writing-markup-with-jsx) (denoted with the file extensions .jsx or .tsx) and convert them into regular JavaScript.

Ultimately, these are good processes to start to understand now. However, if you are struggling with these concepts right now don't worry too much. You will have a unit on the build process where you will learn more about Vite and other build tools later in the program.

Install the necessary dependencies by typing in the following command:

```
npm install
```

After the install has finished, from the main directory, run vite with the following command:

```
npm start
```

1. When you run the above command your browser should automatically open your app on `http://localhost:5173/`

2. Follow the steps below to build Tic-Tac-Toe in the file `src/App.tsx`.

   - **Warning:** Make sure to not skip any steps! 🤨🤨🤨

3. Your changes to `App.tsx` will auto rebuild and reload your application.
