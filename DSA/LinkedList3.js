...
class LinkedList {
  ...
  insertLast(value) {
    ...
    let currentNode; // this.headNode is starting point, make new variable currentNode to not change original list
  ...
  insertAt(value, index) {
    ...
    // If first index // special case: insert at head
    if (index === 0) {
      // index 4 !== 0 so skip
  ...
  getList() {
    ...
    while (current !== null) {
      output.push({ [`Node${counter++}`]: current.value });
      current = current.next;
    }
